...
class PageManager {
    ...
    static async startApp(manifestConfig){
        let pageNavigation = await this.getArrayNavegation();
        console.log("PAGE SAVED",pageNavigation);
        if(pageNavigation.pageNames.length>0){ // Actualizar pagina establecida
            console.log("Current page: ",pageNavigation);
        }
        
        // console.log("SEARCH = ",window.location.search);
        // Validando manifest
        let mainPage = await PageManager.validateManifest(manifestConfig);

        // Guardamos en session la configuración del 
        Store.set('MANIFEST',manifestConfig);
        // Cargando tema
        await Resource.loadTheme(manifestConfig.theme);

        // Buscamos la pagina principal
        await PageManager.configPage();

        await Resource.import(mainPage.name);

        // Colocando la pagina en la URL
        window.location.href = `#/${mainPage.className}`;

        // Iniciamos la actividad principal
        let intent = new Intent(null, mainPage.className);
        let pageInstance = await PageManager.startPage(intent);
        // console.log("Page Instance",pageInstance);

        // let pageSaved = await PageManager.getArrayNavegation();

        // pageSaved.pageNames.push(mainPage.className);
        // // if(pageSaved.length == 0)
        //     window.location.href = `#/${pageSaved.pageNames.join('/')}`;

        // window.location.href = `#/${pageSaved.pageNames.join('/')}`;
    }

    static async validateManifest(manifestConfig){
        let mainPageName = null;
        for(let pageConfig of manifestConfig.pages){
            let posBarra = pageConfig.name.lastIndexOf('/');
            let pageName = pageConfig.name.substring(posBarra+1,pageConfig.name.lastIndexOf('.js'));
            pageConfig.className = pageName;
            if(pageConfig.category=='ROOT')
                mainPageName = pageConfig;
        }
        return mainPageName;
    }

    static async configPage() {
    ...
    static async startPage(intent) {
        ...
        if(!pageConfig){
            throw new Exception(`No se encontro la página [${intent.pageName}] en la configuración del Manifest`);
        }
...
class Page extends Context{
    ...
    async startPage(intent) {
        ...
        let newPage = await PageManager.startPage(intent);
        console.log("SSSS",window.location.href);
        console.log("NUeva",`${window.location.href}/${intent.pageName}`);
