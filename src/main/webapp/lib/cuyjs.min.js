...
class ScrollView extends View{
    ...
    async addEventScroll(){
        console.log("Agregando eventos de SCROLL");
        ...
        this.bodyScroll.ontouchstart=(evTouch=>{
            console.log("TOUCH START",evTouch);
            if(!this.moving)
                return;
            let event = evTouch.touches[0];
            console.log("Post Touch: "+event.clientY);
            this.startMovement(parseInt(event.clientY));
        });
        this.bodyScroll.ontouchmove=(evTouch=>{
            console.log("TOUCH MOVE",evTouch);
            if(!this.moving)
                return;
            let event = evTouch.touches[0];
            console.log("Post Touch MOVE: "+event.clientY);
            let dist = parseInt(event.clientY) - this.starty;
            console.log("DIST TOUCH",dist);
            this.moveContent(dist);
        });
